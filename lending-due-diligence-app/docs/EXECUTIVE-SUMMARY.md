...
is days of work; our **93 to 141 day estimate for an application of this
...
transitions live inside compiled flow rules and are simply absent from the
export. We addressed this with defence in depth - guards inferred from the
preceding decision tables, plus a hard re-visit cap that guarantees termination
even when an inferred guard is wrong. Second, extraction quality is silently
variable; an assessment run without live sandbox access captured **zero**
screens and choice sets while still producing a plausible-looking model. Both
defects were caught by automated verification rather than manual testing, which
is the argument for building the verification harness before the application.

**We recommend proceeding to a Phase 1 production migration**, conditional on
securing two things that sit outside the delivery team: Pega Dev Studio access
to recover the real flow-rule conditions, and business SME time to restate the
transform and notification logic. The suggested next step is a two-week
inception to retire the highest risk before committing to the full build. The
migration assets built here - the extraction pipeline, case engine, dynamic form
renderer and verification harness - are application-agnostic and reusable, so a
second application should cost 40 to 50% less. Three things this PoC
deliberately does **not** claim: performance at production volume, an enforced
security model, and migration of in-flight cases. Each is scoped as Phase 1
work, and the recommended cutover strategy - drain and switch - avoids the last
one entirely.
...
| Estimated Phase 1 effort | 93 - 141 days (~12 weeks, team of 3) |
